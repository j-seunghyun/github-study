      ...
      - Reset
            ...
            * 중요
                  ...
                  * git reset --hard명령을 통해 과거에 돌아갈 때
                        ...
                        - git log는 reset --hard로 인해 삭제되지만 objects에는 절대 사라지지 않는다. 그래서 결국 복원을 위해서 돌아가고 싶은 곳의 SHA1 해시값을 알고 있다면 언제든지 돌아갈 수 있다.

                        - 또한 백업해둔 .git파일로 복원을 시도할 경우 백업해둔 .git파일에 있는 git log가 들어오는 것이므로 복원 시도하기 전 원래 git log와 비교해서 바뀐점이 있는 것에 대해 git status 즉 현재 상태를 알려준다.
